...
use crate::{open_memory, open_store};

pub(crate) async fn gc(path: &Path, format: Format) -> Result<(), Box<dyn std::error::Error>> {
...
pub(crate) async fn forget_adaptive(
    ...
    let store = open_store(path).await?;
    let task = basemyai::AdaptiveForgetting { capacity_per_agent, recency_half_life_secs: half_life_secs };
...
pub(crate) async fn consolidate(path: &Path, agent: &str, format: Format) -> Result<(), Box<dyn std::error::Error>> {
    let memory = open_memory(path, agent).await?;
    let provision = basemyai::choose_llm().await.map_err(|e| {
    ...
    format.print(
        || {
            println!("consolidation via {} — episodes_seen={}", provision.model_id, report.episodes_seen);
            println!(
