...
/// DUR-LSM-01 regression under a real forced kill (BaseMyAI adversarial
/// audit, 2026-07-22): same spawn/kill/reopen machinery, against
/// `crash_writer`'s `"compact-race"` mode, which repeatedly stages a
/// compaction and rewrites an already-in-the-merge's-input key *between*
/// `compact_prepare` and `compact_commit` — the exact interleaving that let
/// a compaction publish stale data ahead of a concurrently-flushed update.
/// See [`run_compact_race_cycles`] for exactly what is checked.
#[test]
fn compact_race_kill_reopen_verify_loop() {
    run_compact_race_cycles(cycles());
}
...
/// Compaction-race kill loop: same spawn/sleep/kill sequence as
/// [`run_cycles`], against `crash_writer`'s `"compact-race"` mode
/// (DUR-LSM-01 regression). Every confirmed `race <slot> <epoch>` line means
/// "the write racing that cycle's compaction, and the compaction itself,
/// are both durable" — so after each kill, every slot that has ever been
/// confirmed must read back an epoch **at least** its last-confirmed one
/// (the kill can land after a commit durably lands but before the
/// confirm-log write for it — same "confirmed ⇒ durable, not durable ⇒
/// confirmed" contract as every other mode here). What must never happen,
/// and is exactly what DUR-LSM-01 could cause, is the *opposite*: a slot
/// reading back **lower** than its last-confirmed epoch — a stale merged
/// value (or an older confirmed epoch) resurfacing after the commit that
/// was supposed to supersede it.
fn run_compact_race_cycles(cycles: u32) {
    let tmp = tempfile::tempdir().expect("tempdir");
    let engine_dir = tmp.path().join("engine");
    let confirm_log = tmp.path().join("confirmed.log");
    let bin = env!("CARGO_BIN_EXE_crash_writer");

    let mut max_confirmed_epoch_seen: [Option<u64>; basemyai_engine::harness::RACE_SLOTS as usize] =
        [None; basemyai_engine::harness::RACE_SLOTS as usize];
    let mut any_cycle_confirmed = false;

    for cycle in 0..cycles {
        let child = spawn_writer(bin, &engine_dir, &confirm_log, "compact-race");
        kill_after_jitter(child, cycle);

        let last_confirmed = read_last_confirmed_race(&confirm_log);
        if last_confirmed.iter().all(Option::is_none) {
            continue;
        }
        any_cycle_confirmed = true;

        let engine = Engine::open(&engine_dir)
            .unwrap_or_else(|e| panic!("cycle {cycle}: engine failed to reopen after kill: {e}"));

        for (slot, confirmed_epoch) in last_confirmed.into_iter().enumerate() {
            let Some(epoch) = confirmed_epoch else { continue };
            assert!(
                max_confirmed_epoch_seen[slot].is_none_or(|prev| epoch >= prev),
                "cycle {cycle}: slot {slot}'s confirmed epoch regressed ({epoch} < {prev:?}) — the \
                 confirmation log itself should only ever grow",
                prev = max_confirmed_epoch_seen[slot]
            );
            max_confirmed_epoch_seen[slot] = Some(epoch);

            let key = basemyai_engine::harness::race_key(slot as u64);
            let got = engine
                .get(&key)
                .unwrap_or_else(|e| panic!("cycle {cycle}: get(race slot {slot}) errored: {e}"));
            let got_epoch = got.as_deref().map(|bytes| {
                u64::from_be_bytes(bytes.try_into().unwrap_or_else(|_| {
                    panic!("cycle {cycle}: race slot {slot} holds a malformed epoch value: {bytes:?}")
                }))
            });
            assert!(
                got_epoch.is_some_and(|got_epoch| got_epoch >= epoch),
                "cycle {cycle}: race slot {slot} was confirmed at epoch {epoch} (a compaction \
                 commit that raced a concurrent write to this same slot completed durably) but \
                 reads back epoch {got_epoch:?} after reopen — DUR-LSM-01 regression: a stale \
                 merged value or an older epoch resurfaced"
            );
        }
        engine
            .close()
    ...
    assert!(
        any_cycle_confirmed,
        "no cycle ever confirmed a single compaction race — the harness or writer is broken, not the engine"
    );
}
...
/// Reads the last well-formed `race <slot> <epoch>` line per slot — kept
/// independent of the writer's own parser for the same trust reasons as
/// [`read_last_confirmed_batch_end`]. Index `i` of the returned array is
/// slot `i`'s last confirmed epoch, `None` if that slot was never confirmed
/// in this log.
fn read_last_confirmed_race(path: &Path) -> [Option<u64>; basemyai_engine::harness::RACE_SLOTS as usize] {
    let mut last = [None; basemyai_engine::harness::RACE_SLOTS as usize];
    ...
    for line in reader.lines().map_while(Result::ok) {
        let mut parts = line.split_whitespace();
        if parts.next() != Some("race") {
            continue;
        }
        let (Some(slot_str), Some(epoch_str), None) = (parts.next(), parts.next(), parts.next()) else {
            continue;
        };
        let (Ok(slot), Ok(epoch)) = (slot_str.parse::<u64>(), epoch_str.parse::<u64>()) else {
            continue;
        };
        if let Some(slot_idx) = last.get_mut(slot as usize) {
            *slot_idx = Some(epoch);
        }
    }
    last
