...
mod candle_impl {
    ...
    impl CandleEmbedder {
        ...
        pub fn load(model_dir: &Path, device: Device) -> Result<Self> {
            ...
            let config_bytes = std::fs::read(&config_path)
                .map_err(|e| CoreError::Embed(format!("lecture de config.json: {e}")))?;
            ...
            let model = BertModel::load(vb, &config)
                .map_err(|e| CoreError::Embed(format!("chargement du modèle BERT: {e}")))?;
    ...
    fn to_candle_device(device: Device) -> candle_core::Device {
        match device {
            ...
            Device::Cuda(index) => {
                candle_core::Device::new_cuda(index).unwrap_or(candle_core::Device::Cpu)
            }
            Device::Metal => {
    ...
    fn stack_u32(rows: &[Vec<u32>], device: &candle_core::Device) -> Result<Tensor> {
        ...
        Tensor::stack(&tensors, 0)
            .map_err(|e| CoreError::Embed(format!("empilage du lot: {e}")))
    }
...
#[cfg(feature = "embed")]
pub use candle_impl::CandleEmbedder;
