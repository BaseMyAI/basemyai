...
| [x] | Exécution stress 1h + mémoire | Run 55 min continu (`SECS=3300`) avec monitoring OS (`WorkingSet64`, ~30s) : `test result: ok`, mémoire bornée ~61-193MB sans dérive. Brut archivé dans `docs/benchmarks/m6-candle-stress-results-2026-07-01.md`. |
...
| [x] | Résultats KNN 10k/100k/1M | 10k + 100k réels archivés dans `docs/benchmarks/m6-knn-results-2026-07-01.md` (+ finding coût de build d'index natif). 1M extrapolé (~22h de build d'index estimé), pas exécuté cette session — raisonnement documenté dans le même fichier. |
...
| [x] | Key rotation (chiffrement) | `Store::rotate_key` (`crates/basemyai-core/src/storage/store.rs`, feature `crypto`) + `Memory::rotate_key` (`crates/basemyai/src/memory/mod.rs`). `PRAGMA rekey` en place (bascule `journal_mode` DELETE→WAL, requis par SQLite3MultipleCiphers). Le `Store` ayant exécuté la rotation doit être rouvert ensuite (pool de lecteurs + `encryption_config` de `libsql::Database` figés à l'ancienne clé). Tests : `crates/basemyai-core/tests/store.rs`, `crates/basemyai/tests/key_rotation.rs`. |
...
- [x] REST : `GET /v1/watch` (SSE), relaie `Memory::watch` en JSON (un événement par ligne `data:`), auth Bearer réutilisée, arrêt propre au disconnect (pas de tâche spawnée — `stream::unfold` droppe la `MemorySubscription` quand axum arrête de poller). Isolation testée par un test adversarial. `crates/basemyai-rest/src/routes.rs` + `tests/api.rs` (`watch_delivers_remembered_event_for_same_agent`, `watch_isolates_events_from_other_agents`).
- [x] MCP : outil `watch` démarre un relais en `notifications/message` (`logger="basemyai.memory"`) poussé via le `peer` de la session (même mécanisme push que `SamplingBackend`), tâche de fond qui s'arrête d'elle-même dès que l'envoi échoue (session fermée). Isolation testée par un test adversarial. `crates/basemyai-mcp/src/server.rs` + `tests/watch.rs` (`watch_delivers_remembered_notification_for_same_agent`, `watch_isolates_notifications_from_other_agents`).
- [x] PyO3 : `Memory.watch(layer=None)` rend un itérateur asynchrone (`async for event in memory.watch(...)`, conforme à PLAN.md §P2.1), `bindings/basemyai-py/src/memory.rs` (+ `types.rs`, classe `WatchEvent`), tests verts (`tests/test_roundtrip.py`, build+pytest vérifiés via `maturin develop --features test-util`). **Caveat connu, non résolu** : annuler un `__anext__()` en attente (ex. `asyncio.wait_for(anext(watcher), timeout=...)`) provoque un access violation Windows différé (crash observé sur le test suivant) — cohérent avec le hasard déjà documenté sur l'abandon de tâches tokio en plein I/O (`crates/basemyai-mcp/tests/sampling.rs`). Le test d'isolation a été écrit pour l'éviter (pas de cancellation) ; consommer l'itérateur jusqu'au bout plutôt que le timeout-canceller tant que ce n'est pas creusé.
- [ ] NAPI (Node) : reporté faute de temps — napi-rs n'a pas d'équivalent direct à l'itérateur asynchrone Python (protocole `Symbol.asyncIterator`) ; porter `Memory::watch` demanderait soit un `ThreadsafeFunction` en callback (style `EventEmitter`), soit une classe JS manuelle implémentant `[Symbol.asyncIterator]` par-dessus un `next()` façon PyO3 — non commencé, `bindings/basemyai-node/src/memory.rs` inchangé.
