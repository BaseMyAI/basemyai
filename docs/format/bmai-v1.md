...
`Memory::open` also writes `embedding_model_id` on first open with the supplied
embedder. Reopening a container with a different `model_id` or dimension is
rejected so callers do not silently query vectors generated by another model.
Use JSONL export/import to re-embed the memory with a new model.
