...
- [x] `put_memory_batch` tout-ou-rien : `PersistentVectorIndex::insert_many_with`
      ...
      lot — _avant_ d'écrire quoi que ce soit, alloue les `vec_id` séquentiels,
...
NativeInner>>` à `Arc<RwLock<NativeInner>>` : les lectures pures
      ...
      mono-écrivain sync, ADR-025 inchangée) — lever _ça_ exigerait un moteur
...
- [x] ADR de bascule du défaut libSQL→Native (chiffres à l'appui) —
      décision séparée et humaine, jamais prise en passant ( décision prise supprimé libsql est aller sur le natif)
